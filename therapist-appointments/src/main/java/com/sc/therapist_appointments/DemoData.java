...
public class DemoData {
    ...
    static SolverConfig solverConfig = new SolverConfig().withSolutionClass(
                                                                 Schedule.class)
                                                         .withEntityClasses(
                                                                 Appointment.class)
...
//                                                         .withEnvironmentMode(EnvironmentMode.FULL_ASSERT)
                                                         ...
                                                         .withTerminationSpentLimit(
                                                                 Duration.ofSeconds(
                                                                         5));
    static SolverManager<Schedule, UUID> solverManager = SolverManager.create(
            solverConfig,
    ...
    public static List<Timeslot> generateTimeslots(LocalDate startDate, LocalDate endDate, LocalTime startTime, LocalTime endTime) {
        ...
        while (!currentDate.isAfter(endDate)) {
            ...
            while (!currentTime.isAfter(endTime)) {
                timeslotList.add(new Timeslot(currentDate,
                                              currentTime,
...
//        LatLng locs = new LatLng(17.3850, 78.4867); // Hyderabad
        ...
        Patient patient1 = new Patient("Patient 1",
                                       ...
                                       timeslotList.stream()
                                                   .filter(x -> x.getDate()
                                                                 .getDayOfMonth() == 11 && x.getStartTime()
                                                                                            .isBefore(
                                                                                                    LocalTime.of(
                                                                                                            12,
                                                                                                            00)))
        ...
        Patient patient2 = new Patient("Patient 2",
                                       ...
                                       timeslotList.stream()
                                                   .filter(x -> x.getDate()
                                                                 .getDayOfMonth() == 12 && x.getStartTime()
                                                                                            .isAfter(
                                                                                                    LocalTime.of(
                                                                                                            12,
                                                                                                            00)))
        ...
        List<Therapist> therapists = Arrays.asList(therapist1,
                                                   therapist2,
                                                   therapist3,
                                                   therapist4,
                                                   therapist5);
        ...
        schedule.setAppointmentList(schedule.getPatientList()
                                            .stream()
                                            .map(Appointment::new)
                                            .toList());
    ...
    public static Schedule runSolver() throws ExecutionException, InterruptedException {
        ...
        ScoreAnalysis<HardSoftScore> scoreAnalysis = solutionManager.analyze(
                solvedSchedule);
    ...
    private static void printSchedule(Schedule schedule) {
        ...
        for (Timeslot timeslot : schedule.getTimeslotList()) {
            List<List<Appointment>> cellList = roomList.stream().map(room -> {
                Map<Patient, List<Appointment>> byRoomMap = appointmentMap.get(
                        timeslot);
                if (byRoomMap == null) {
